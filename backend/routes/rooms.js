...
import { calculateAvailableSlots, summarizeRoomUsage } from '../lib/slots.js'
...
export default async function roomsRoutes(fastify) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      // Active bookings for this meeting, grouped by room, to compute usage that
      // accounts for the inter-meeting buffer and minimum bookable length.
      ...
      return rows.map((room) => {
        ...
        const usage = summarizeRoomUsage(room, meeting, roomBookings, meeting.buffer)
        return {
          ...room,
  ...
  fastify.put(
    ...
    async (request, reply) => {
      ...
      const [updated] = await db.update(rooms).set(updateData).where(eq(rooms.id, id)).returning()
